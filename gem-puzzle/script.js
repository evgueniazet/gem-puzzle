...
const renderCell = (event) => {
  ...
  if (isValid) {
    ...
    setTimeout(() => {
      if (isWon(cellMatrix)) {
        headerInfoTime.textContent =
          +(min > 9 ? min : "0" + min) + ":" + (sec > 9 ? sec : "0" + sec);
        clearTimeout(t);
        alert(
          `Hooray! You solved the puzzle in ${headerInfoTime.textContent} and ${counterMoves} moves!`
        );
      }
    }, 300);
  }
...
const findCoordinatesByNumber = (number, cellMatrix) => {
...
const swap = (cellCoords, emptycellCoords, cellMatrix) => {
  const coords1Number = cellMatrix[cellCoords.y][cellCoords.x];
  cellMatrix[cellCoords.y][cellCoords.x] =
    cellMatrix[emptycellCoords.y][emptycellCoords.x];
  cellMatrix[emptycellCoords.y][emptycellCoords.x] = coords1Number;
};

const getMatrix = (shuffleArray) => {
  const matrix = [[], [], [], []];
  let y = 0;
  let x = 0;
